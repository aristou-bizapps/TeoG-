...
==============================================================================================================
1       HS      2026-01-26      Create new "Statement of Account" report (copied from 85010 "ATU_Customer Statement (Email)")
2                               Make some change from user's request
*/

//HS.1+
report 50009 "ATU_Customer Statement (Email)"
{
    ApplicationArea = All;
    Caption = 'ATU Customer Statement (Email)';
    DefaultRenderingLayout = "ATU_Customer Statement (Email)";
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            column(RegNo_CompanyInfo; CompanyInfo."Registration No.")
            {
            }
            // End of added fields
            //HS.2+
            ...
            column(ATU_PaymentTerm; "Payment Terms Code") { }
            column(ATU_AsAtCaption; StrSubstNo('As at: %1', Format(EndDate, 0, '<Day,2> <Month Text> <Year4>'))) { }
            //HS.2-
            dataitem("Integer"; "Integer")
            {
                ...
                column(EndDate; Format(EndDate, 0, 4))
                {
                }
                ...
                dataitem(CurrencyLoop; "Integer")
                {
                    ...
                    dataitem(CustLedgEntryHdr; "Integer")
                    {
                        ...
                        {
                            DataItemTableView = SORTING("Customer No.", "Posting Date", "Entry Type", "Currency Code") order(ascending);
                            column(PostDate_DtldCustLedgEntries; Format("Posting Date", 0, '<Closing><Day,2>/<Month,2>/<Year>'))
                            {
                            }
                            ...
                            column(DueDate_DtldCustLedgEntries; Format(DueDate, 0, '<Closing><Day,2>/<Month,2>/<Year>'))
                            {
                            }
                            ...
                            //{<<<<<<<<<<<<<<<<<<<<<<<<<<< IC - 2023/03/29 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>>}
                            column(SUMRemainAmt_DtldCustLedgEntries; SUMRemainingAmount)
                            {
                                AutoFormatExpression = "Currency Code";
                                AutoFormatType = 1;
                            }
                            //{<<<<<<<<<<<<<<<<<<<<<<<<<<< IC - 2023/03/29 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>>>}
                            ...
                            var
                                RecDtldCustLedgEntries: Record "Detailed Cust. Ledg. Entry";
                            begin
                                ...
                                RecDtldCustLedgEntries.Reset();
                                RecDtldCustLedgEntries.CopyFilters(DtldCustLedgEntries);
                                RecDtldCustLedgEntries.SetRange("Entry Type", "Entry Type"::"Initial Entry");
                                IF RecDtldCustLedgEntries.FindSet() then begin
                                    repeat
                                        CustLedgerEntry.Get(RecDtldCustLedgEntries."Cust. Ledger Entry No.");
                                        CustLedgerEntry.SetRange("Date Filter", 0D, EndDate);
                                        CustLedgerEntry.CalcFields("Remaining Amount");
                                        SUMRemainingAmount += CustLedgerEntry."Remaining Amount";
                                        CustLedgerEntry.SetRange("Date Filter");
                                    until RecDtldCustLedgEntries.Next() = 0;
                                end;
                                //{<<<<<<<<<<<<<<<<<<<<<<<<<<< IC - 2023/03/29 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>>>}
                            end;
                        }
                    ...
                    dataitem(OverdueVisible; "Integer")
                    {
                        ...
                        dataitem(CustLedgEntry2; "Cust. Ledger Entry")
                        {
                            ...
                            column(PostDate_CustLedgEntry2; Format("Posting Date", 0, '<Closing><Day,2>/<Month,2>/<Year>'))
                            {
                            }
                            ...
                            column(DueDate_CustLedgEntry2; Format("Due Date", 0, '<Closing><Day,2>/<Month,2>/<Year>'))
                            {
                            }
                ...
                dataitem(AgingBandVisible; "Integer")
                {
                    ...
                    dataitem(AgingBandLoop; "Integer")
                    {
                        ...
                        column(AgingBandEndingDate; StrSubstNo(AgedSummaryLbl, Format(AgingBandEndingDate, 0, '<Closing><Day,2>/<Month,2>/<Year>'), PeriodLength, SelectStr(DateChoice + 1, DuePostingDateLbl)))
                        {
                        }
                        ...
                        //{<<<<<<<<<<<<<<<<<<<<<<<<<<< IC - 2023/03/29 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>>}
                        column(AgingBandBufCol1until5Amt; (TempAgingBandBuf."Column 1 Amt." + TempAgingBandBuf."Column 2 Amt." + TempAgingBandBuf."Column 3 Amt." + TempAgingBandBuf."Column 4 Amt." + TempAgingBandBuf."Column 5 Amt."))
                        {
                            AutoFormatExpression = TempAgingBandBuf."Currency Code";
                            AutoFormatType = 1;
                        }
            ...
            begin
                ...
                ATU_gReportMgmt.ATU_GetCustomerAddress("No.", ATU_gCustomerAddress);
                //HS.2-
    ...
    rendering
    {
        layout("ATU_Customer Statement (Email)")
        {
            Type = Word;
            LayoutFile = './src/report layout/RL_50009_ATU_Customer Statement (Email).docx';
        }
    }
    ...
    {
        //HS.2+
        ATU_ReportCaption = 'STATEMENT OF ACCOUNT';
    ...
    begin
        ...
        g_user.Reset();
        g_user.SetCurrentKey("User Name");
        g_user.SetRange("User Name", UserId);
        if g_user.FindFirst() then;

        //{<<<<<<<<<<<<<<<<<<<<<<<<<<< IC - 2023/03/29 - START >>>>>>>>>>>>>>>>>>>>>>>>>>>>}
        CompanyInfo.Get();
        BodyLbl := 'Thank you for being a valued customer. We appreciate your business and want to ensure that you are always up-to-date on what you owe. Please find attached your latest statement of account. If you have any questions, please do not hesitate to contact us: ' + g_user."Contact Email" + '. Thank you for your time, and have a great day!';
        //{<<<<<<<<<<<<<<<<<<<<<<<<<<< IC - 2023/03/29 - FINISH >>>>>>>>>>>>>>>>>>>>>>>>>>>>}
    end;
    ...
    var
        ...
        SUMRemainingAmount: Decimal; // Added by IC
    ...
    /// </summary>
    /// <param name="DetailedCustLedgEntry">VAR Record "Detailed Cust. Ledg. Entry".</param>
    ...
    /// <param name="NewDateChoice">Option "Due Date","Posting Date".</param>
