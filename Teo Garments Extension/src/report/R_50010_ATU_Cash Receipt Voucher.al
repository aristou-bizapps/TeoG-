...
==============================================================================================================
1       HS      2026-01-28      Create new "Cash Receipt Voucher" report (copied from GR 50105 - "GoldRoast Receipt Voucher")
2                               Make some change from user's request
*/

//HS.1+
report 50010 "ATU_Cash Receipt Voucher"
{
    Caption = 'ATU Cash Receipt Voucher';
    DefaultRenderingLayout = "ATU_Cash Receipt Voucher";
    ...
    dataset
    {
        dataitem("Gen. Journal Batch"; "Gen. Journal Batch")
        {
            ...
            column(CompanyTelNo; CompanyInformation."Phone No.")
            {
            }
            column(CompanyFaxNo; CompanyInformation."Fax No.")
            {
            }
            column(CompanyRegNo; CompanyInformation."Registration No.")
            {
            }
            // End of added fields
            dataitem("Integer"; "Integer")
            {
                ...
                dataitem("Gen. Journal Line"; "Gen. Journal Line")
                {
                    ...
                    column(ATU_CustAddr1; ATU_gCustomerAddress[1]) { }
                    column(ATU_CustAddr2; ATU_gCustomerAddress[2]) { }
                    column(ATU_CustAddr3; ATU_gCustomerAddress[3]) { }
                    column(ATU_CustAddr4; ATU_gCustomerAddress[4]) { }
                    column(ATU_CustAddr5; ATU_gCustomerAddress[5]) { }
                    column(ATU_CustAddr6; ATU_gCustomerAddress[6]) { }
                    column(ATU_CustAddr7; ATU_gCustomerAddress[7]) { }
                    column(ATU_PreparedPostedBy; UserId) { }
                    column(ATU_AmountReceiptCaption; StrSubstNo('Amount Receipt (%1)', ATU_gReportMgmt.ATU_GetCurrencyCode("Currency Code"))) { }
                    //HS.2-
                    column(PostingDate_GenJnlLine; Format("Posting Date", 0, '<Day,2>-<Month Text,3>-<Year4>'))
                    {
                    }
                    ...
                    column(AmountLCY_GenJnlLine; "Amount (LCY)" * -1)
                    {
                    }
                    ...
                    dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
                    {
                        ...
                        DataItemLink = "Applies-to ID" = field("Document No."), "Customer No." = field("Account No."); // <-- 15-NOV-2022
                        DataItemTableView = sorting("Entry No.");

                        column(CustLedgerEntry_EntryNo; "Entry No.")
                        {
                        }
                        column(CustLedgerEntry_PostingDate; Format("Posting Date", 0, '<Closing><Day,2>/<Month,2>/<Year4>'))
                        {
                        }
                        column(CustLedgerEntry_DocumentNo; "Document No.")
                        {
                        }
                        column(CustLedgerEntry_Description; Description)
                        {
                        }
                        column(CustLedgerEntry_CurrencyCode; "Currency Code")
                        {
                        }
                        /* column(CustLedgerEntry_RemainingAmount; ReportHelper.CalcFCY("Gen. Journal Line"."Currency Factor", "Remaining Amount")) // <-- 18-Oct-2022
                        {
                        } */
                        column(CustLedgerEntry_RemainingAmount; ATU_gReportMgmt.ATU_CalculateFCY("Gen. Journal Line"."Currency Factor", "Remaining Amount")) // <-- 18-Oct-2022
                        {
                        }
                        column(CustLedgerEntry_AmountPaid; (Amount - "Remaining Amount"))
                        {
                        }
                        column(CustLedgerEntry_AmountPaidLCY; ("Amount (LCY)" - "Remaining Amt. (LCY)"))
                        {
                        }
                        /* column(CustLedgerEntry_AmounttoApply; ReportHelper.CalcFCY("Gen. Journal Line"."Currency Factor", "Amount to Apply")) { } // <-- 18-Oct-2022
                        column(CustLedgerEntry_AmounttoApplyLCY; ReportHelper.CalcApplnAmountToApply("Amount to Apply", "Posting Date", "Currency Code", "Posting Date")) // CalcApplnAmountToApply
                        {
                        } */
                        column(CustLedgerEntry_AmounttoApply; ATU_gReportMgmt.ATU_CalculateFCY("Gen. Journal Line"."Currency Factor", "Amount to Apply")) { } // <-- 18-Oct-2022
                        column(CustLedgerEntry_AmounttoApplyLCY; ATU_gReportMgmt.ATU_CalculateApplnAmountToApply("Amount to Apply", "Posting Date", "Currency Code", "Posting Date")) // CalcApplnAmountToApply
                        {
                        }

                        trigger OnAfterGetRecord()
                        begin
                            "Cust. Ledger Entry".CalcFields(Amount, "Amount (LCY)", "Remaining Amount", "Remaining Amt. (LCY)");
                        end;
                    }
                    // End of added DataItem

                    // Added fields
                    column(LocalCurrencyCode; GLSetup."LCY Code")
                    {
                    }
                    ...
                    column(TotalAmtInText; TotalAmtInText)
                    {
                    }
                    // End of added fields
                    ...
                    // AristouReportFunctions: Codeunit "Aristou Report Functions";
                    // End of added vars
                    begin
                        ...
                        if ("Account Type" = "Account Type"::Customer) and ("Account No." <> '') then
                            ATU_gReportMgmt.ATU_GetCustomerAddress("Account No.", ATU_gCustomerAddress);
                        //HS.2-

                        // TotalAmtInText := AristouReportFunctions.FormatNoText("Amount (LCY)", GLSetup."LCY Code", 'CENTS');
                        // TotalAmtInText := AristouReportFunctions.FormatNoText("Amount (LCY)", "Gen. Journal Line"."Currency Code", 'CENTS');
                        TotalAmtInText := ATU_gConvertAmtInWord.ATU_FormatNoText("Amount (LCY)", ATU_gReportMgmt.ATU_GetCurrencyCode("Currency Code"), 'CENTS');
    ...
    rendering
    {
        layout("ATU_Cash Receipt Voucher")
        {
            Type = RDLC;
            LayoutFile = './src/report layout/RL_50010_ATU_Cash Receipt Voucher.rdl';
        }
    }
    ...
    {
        //HS.2+
        ATU_ReportCaption = 'RECEIPT VOUCHER';
        ATU_RVNoCaption = 'RV NO.';
        ATU_DateCaption = 'DATE';
        ATU_BankCodeCaption = 'BANK CODE';
        ATU_ChequeNoCaption = 'CHEQUE NO.';
        ATU_CustomerNoCaption = 'CUSTOMER NO.';
        ATU_CustomerAddressCaption = 'Customer Address:';
        ...
        ATU_PreparedPostedByCaption = 'Prepared & Posted by';
        //HS.2-
    }
    ...
    var
        ...
        TotalAmtInText: Text;
        // ReportHelper: Codeunit "GR Reporting Helper";
        // End of added vars
        //HS.2+
        ...
        ATU_gCustomerAddress: array[7] of Text[150];
    //HS.2-
    ...
    begin
        if not BankAcc.Get(GenJnlLine."Account No.") then
            ...
            if GenJnlLine."Bank Payment Type" <> "Bank Payment Type"::" " then
    ...
    local procedure TestPostingType()
    begin
        case true of
                            ...
                            if ICGLAccount.Get("Gen. Journal Line"."IC Account No.") then
                                if ICGLAccount.Blocked then
                                    AddError(
                                      StrSubstNo(
                                        Text032,
                                        ICGLAccount.FieldCaption(Blocked), false, "Gen. Journal Line".FieldCaption("IC Account No."),
    ...
    local procedure InitializeRequest(NewShowDim: Boolean)
    begin
        ShowDim := NewShowDim;
    end;
